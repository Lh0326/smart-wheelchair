...
1. EEG 采集：ADS1299 八通道采集前端以 500Hz 采样（braincontrol_tab.py:92-93），经 1Mbps 串口传输至主机（braincontrol_tab.py:86）；特征提取阶段使用四个核心通道 F3/FzL/FzR/F4（feature_extractor.py:21-23），与训练特征空间一致。
...
上图是导航前端（Web 地图）在滞回切换瞬间前的真实状态：处于偏离路径较远的 APPROACHING 阶段，界面以红色目标点标记最近路径接入点，引导轮椅先回到路网再开始跟踪——这正是 5 米触发阈值生效时的用户视角。
...
上图对上：进入 ON_PATH 跟踪后，RViz 中的路径与目标点同屏——当前目标（蓝色标记）始终锁定路径前方最近的拐角而非终点，L 形路段因此先直行再转向，不会发生指向终点的"窜路"。
